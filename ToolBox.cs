...
namespace ReportDesigner
{
		...
		private void PaintItems(Graphics g, Rectangle clipRect,StringFormat strFormat)
		{
			...
			if(! (0 >= _itemArea.Height)  && null != _toolItems)
			{
				...
				for(iLoop=_visibleTopIndex;iLoop<_visibleBottomIndex;iLoop++)
				{
					...
					rect    = item.Rectangle;
					rect.Y += _itemArea.Y;
                    rect.X = _itemArea.X + _parent.ImageWidth;
	...
	public class ToolBox : UserControl 
	{
        ...
        private bool _mshowscrollbutton=true;
        ...
        {
            get { return _mshowscrollbutton; }
            set { _mshowscrollbutton = value; }
        }
		[Category("ToolBox")]
