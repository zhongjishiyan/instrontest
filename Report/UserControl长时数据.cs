...
{
    public partial class UserControl长时数据 : UserControl
    {
        ...
        {
            int i;
            CComLibrary.GlobeVal.filesave.mlongdata.Clear();
            for (i = 0; i < this.lstinclude.Items.Count; i++)
            {
                CComLibrary.GlobeVal.filesave.mlongdata.Add(this.lstinclude.list[i].Clone() as ClsStaticStation.ItemSignal );
            }
        }
        public  void Init(int sel)
        {
            ...
            for (int j = 0; j < ClsStaticStation.m_Global.mycls.allsignals.Count; j++)
            {
                ...
                if (CComLibrary.GlobeVal.filesave.mlongdata== null)
                {
                   lstavail.AddItem(ClsStaticStation.m_Global.mycls.allsignals[j]);
                }
                else
                {
                    if (this.lstinclude.MatchItem(CComLibrary.GlobeVal.filesave.mlongdata,
            ...
            for (int j = 0; j < CComLibrary.GlobeVal.filesave.mlongdata.Count; j++)
            {


                lstinclude.AddItem(CComLibrary.GlobeVal.filesave.mlongdata[j]);
